    ...
    <div>
      <header className="bg-neutral-800 text-amber-50 px-3 py-1">
        <h1 className="font-bold text-2xl">
          POS<span className="text-5xl">T</span>S
        </h1>
      </header>
        ...
        {posts.length > 0 ? (
          posts.map((e, idx) => {
            return (
              <div
                key={idx}
                className="flex rounded-2xl gap-2 px-2 py-5 shadow-2xl  flex-col w-1/2 h-1/3"
              >
                <img
                  className="  border rounded-2xl  overflow-hidden object-fill"
                  src={e.image}
                  alt=""
                />
                <div className="flex flex-row justify-between items-center">
                  ...
                  <button
                    ...
                    className=" hover:shadow-xl border rounded-2xl bg-amber-600 cursor-pointer text-lg"
                  >
                    🗑️
                  </button>
                </div>
